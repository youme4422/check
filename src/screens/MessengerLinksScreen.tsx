...
export function MessengerLinksScreen() {
  ...
  const openTelegramChat = async () => {
    const urls = [`tg://resolve?domain=${TELEGRAM_BOT_USERNAME}`, `https://t.me/${TELEGRAM_BOT_USERNAME}`];
  ...
  const handleConnectChannel = async (channel: 'line' | 'telegram') => {
    ...
    try {
      ...
      if (isConnected) {
        ...
        Alert.alert(ui.notConnectedTitle, channel === 'line' ? ui.notConnectedLineBody : ui.notConnectedTelegramBody);
      }
    } catch {
          ...
          <View style={styles.channelBlock}>
            ...
            {generatedCode.line ? (
              <View style={styles.codeBox}>
                ...
                <AppButton label={ui.copyLineCodeButton} variant="secondary" onPress={() => void handleCopyCode('line')} />
                <AppButton
            ...
            {generatedCode.telegram ? (
              <View style={styles.codeBox}>
                ...
                <AppButton
                  ...
                  variant="secondary"
                  onPress={() => void handleCopyCode('telegram')}
                />
                <AppButton
...
function getMessengerUiText(locale: Locale) {
  const uiKo = {
    ...
    codeReadyCopiedBody: (channel: 'line' | 'telegram', bot: string, code: string) =>
      `코드가 복사되었습니다. ${channel === 'line' ? 'LINE' : `Telegram (@${bot})`} 채팅에 붙여넣어 보내주세요.\n${code}`,
  ...
  const uiJa = {
    ...
    connectedTitle: '接続完了',
    connectedLineBody: 'LINE 連携を確認しました。',
    connectedTelegramBody: 'Telegram 連携を確認しました。',
    notConnectedTitle: 'まだ接続されていません',
    notConnectedLineBody: '生成したコードを LINE チャットに送信してから、もう一度接続確認を押してください。',
    notConnectedTelegramBody: '生成したコードを Telegram チャットに送信してから、もう一度接続確認を押してください。',
    connectCheckFailedTitle: '接続確認に失敗しました',
    connectCheckFailedBody: 'チャネルの接続状態を確認できませんでした。',
    ...
    generateLineCodeButton: 'LINEコードを生成',
    generateTelegramCodeButton: 'Telegramコードを生成',
    copyLineCodeButton: 'LINEコードをコピー',
    copyTelegramCodeButton: 'Telegramコードをコピー',
    checkLineConnectionButton: 'LINE接続を確認',
    checkTelegramConnectionButton: 'Telegram接続を確認',
    checkingButton: '確認中...',
    codeLabel: 'コード',
  ...
  const uiEs = {
    ...
    connectCheckFailedTitle: 'Error al comprobar conexión',
    ...
    checkLineConnectionButton: 'Comprobar conexión LINE',
    checkTelegramConnectionButton: 'Comprobar conexión Telegram',
  ...
  const uiZh = {
    ...
    connectedTitle: '连接完成',
    connectedLineBody: '已确认 LINE 连接。',
    connectedTelegramBody: '已确认 Telegram 连接。',
    notConnectedTitle: '尚未连接',
    notConnectedLineBody: '请先在 LINE 聊天中发送生成的代码，然后再次点击连接确认。',
    notConnectedTelegramBody: '请先在 Telegram 聊天中发送生成的代码，然后再次点击连接确认。',
    connectCheckFailedTitle: '连接确认失败',
    ...
    checkLineConnectionButton: '确认 LINE 连接',
    checkTelegramConnectionButton: '确认 Telegram 连接',
    checkingButton: '确认中...',
    codeLabel: '代码',
  ...
  if (locale === 'ko') return uiKo;
  if (locale === 'ja') return uiJa;
